...
function M.copy(env, action)
	...
	return Util.executeSafe(env, string.format("cp %s %s %s", flags, src, dst), action.stderr_hint)
end

function M.copy_exact(env, action)
	local src = Util.resolve(env, action.src)
	if not env.ctx.fs:getInfo(src) then
		error(Util.resolve(env, action.message) or ("Missing source for copy_exact: " .. src))
	end
...
function M.set_executable(env, action)
	...
	return Util.executeSafe(env, string.format("chmod +x %q", path), action.stderr_hint)
end
...
function M.remove(env, action)
	local path = Util.resolve(env, action.path)
	return Util.executeSafe(env, string.format("rm -f %q", path), action.stderr_hint)
end
