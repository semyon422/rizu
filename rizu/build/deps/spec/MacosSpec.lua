...
local function add_ffmpeg(spec, deps, prefix, prefix_abs, tc_bin)
	...
	table.insert(spec.steps, {
		...
		actions = {
			...
			{type = "shell", dir = extract, command = "mkdir -p " .. prefix .. "/ffmpeg; TC=$(ls " .. tc_bin .. "/x86_64-apple-darwin*-clang 2>/dev/null | head -n1); HOST=$(basename $TC | sed 's/-clang$//'); AR=" .. tc_bin .. "/$HOST-ar; RANLIB=" .. tc_bin .. "/$HOST-ranlib; export PATH=" .. tc_bin .. ":$PATH; ./configure --prefix=" .. prefix_abs .. "/ffmpeg --enable-cross-compile --target-os=darwin --arch=x86_64 --cc='$TC' --ar='$AR' --ranlib='$RANLIB' --enable-shared --disable-static --disable-programs --disable-doc --disable-debug --disable-asm --disable-videotoolbox && make -j$(nproc) && make install STRIP=true"},
...
local function add_zlib(spec, deps, prefix, prefix_abs, tc_bin)
	...
	table.insert(spec.steps, {
		...
		actions = {
			...
			{type = "extract", format = "tar.gz", archive = archive, dest = extract, skip_if_exists = true},
			{type = "shell", command = "TC=$(ls " .. tc_bin .. "/x86_64-apple-darwin*-clang 2>/dev/null | head -n1); bash -lc 'cd " .. extract .. " && '$TC' -dynamiclib -fPIC -O2 -DHAVE_UNISTD_H -install_name @rpath/libz.dylib adler32.c crc32.c deflate.c infback.c inffast.c inflate.c inftrees.c trees.c zutil.c compress.c uncompr.c gzclose.c gzlib.c gzread.c gzwrite.c -o " .. prefix_abs .. "/lib/libz.dylib'; cp -f " .. extract .. "/zlib.h " .. prefix .. "/include/zlib.h; cp -f " .. extract .. "/zconf.h " .. prefix .. "/include/zconf.h"},
...
local function add_iconv(spec, deps, prefix, prefix_abs, tc_bin)
	...
	table.insert(spec.steps, {
		...
		actions = {
			...
			{type = "shell", command = "TC=$(ls " .. tc_bin .. "/x86_64-apple-darwin*-clang 2>/dev/null | head -n1); HOST=$(basename $TC | sed 's/-clang$//'); AR=" .. tc_bin .. "/$HOST-ar; RANLIB=" .. tc_bin .. "/$HOST-ranlib; bash -lc 'export PATH=" .. tc_bin .. ":$PATH; cd " .. extract .. " && ac_cv_path_lt_DD=${root_abs}/${deps_dir}/dd32.sh DD=${root_abs}/${deps_dir}/dd32.sh CC='$TC' AR='$AR' RANLIB='$RANLIB' ./configure --host=$HOST --prefix=" .. prefix_abs .. " --enable-shared --disable-static CFLAGS=\"-O2 -fPIC\" < /dev/null && make -j$(nproc) && make install'"},
...
local function add_openssl(spec, deps, prefix, prefix_abs, tc_bin)
	...
	table.insert(spec.steps, {
		...
		actions = {
			...
			{type = "extract", format = "tar.gz", archive = archive, dest = extract, skip_if_exists = true},
			{type = "shell", command = "TC=$(ls " .. tc_bin .. "/x86_64-apple-darwin*-clang 2>/dev/null | head -n1); HOST=$(basename $TC | sed 's/-clang$//'); AR=" .. tc_bin .. "/$HOST-ar; RANLIB=" .. tc_bin .. "/$HOST-ranlib; bash -lc 'export PATH=" .. tc_bin .. ":$PATH; cd " .. extract .. " && CC='$TC' AR='$AR' RANLIB='$RANLIB' ./Configure darwin64-x86_64-cc shared --prefix=" .. prefix_abs .. " --openssldir=" .. prefix_abs .. "/ssl && make -j$(nproc) && make install_sw'"},
