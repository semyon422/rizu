...
function test.get_state_returns_settings_and_selected_note_sound(t)
	...
	t:tdeq(state.toolHotkeys, {"q", "w"})
	t:eq(state.toolHotkeyLabel, "Use q:Select w:ShortNote")
	t:eq(state.logSpeedLabel, "speed 12")
	t:eq(state.snapLabel, "snap 8")
...
function test.set_log_speed_and_tool_hotkeys(t)
	...
	t:eq(service:getToolForHotkey({"Select"}, "w"), nil)
...
---@param t testing.T
function test.handle_input_mutates_settings_and_applies_hotkeys(t)
	local calls = {}
	local editor = {
		snap = 4,
		lockSnap = false,
	...
	t:eq(context:getLogSpeed(), 24)
	t:eq(editor.snap, 16)
	t:eq(editor.lockSnap, true)
	t:eq(editor.tool, "LongNote")
	t:tdeq(calls, {"speed:24"})
end
