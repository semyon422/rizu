...
Playfield note interactions are routed through `EditorPlayfieldService` and `EditorPlayfieldContext`: playfield activation, note-tool/select-tool checks, selection rectangle lookup, add-note commands, note select/grab/remove commands, grabbed-note drop, and selection end. Overlay tabs expose controls, but they do not disable playfield editing; the active editor tool controls whether clicks place notes or start selection. Mouse decision routing is owned by `EditorPlayfieldInputService`: `EditorPlayfieldView` supplies retained mouse-event facts, and the service turns them into playfield commands. Hit-test fact construction is isolated in `EditorPlayfieldHitTestService`, which reads note hover facts from note rendering and column bounds from the active note skin.
