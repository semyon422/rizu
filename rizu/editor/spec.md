...
### ADR: Incremental Retained UI Migration
- **Context**: The editor is still rendered through legacy immediate-mode view functions, while the new `yi` UI is retained and view-tree based.
- **Decision**: Lower-risk editor chrome migrates first by attaching retained `gui.View` wrappers during editor enter. The wrappers currently delegate to service-backed legacy draw functions, which preserves behavior while moving lifecycle, ordering, and future replacement points into the retained framework.
- **Consequence**: Overlay tabs, footer controls, chart slider, waveform, and onset debug panels are now retained screen views attached by `EditorScreenLoadService` and drawn through `yi.Screen` by `EditorScreenFrameService`. The playfield, snap grid, sequence view, and foreground hotkeys remain legacy until their input/render coupling is audited.

### Phase 5 Playfield Migration Analysis
Before migrating the playfield/snap-grid/foreground path to retained UI, resolve these points:
- **Input ownership**: `SnapGridView`, `Foreground`, `SequenceView`, and editor note dragging still combine `just`, LÖVE mouse state, noteskin transforms, and service commands. Phase 5 should define retained drag/hover/focus ownership before replacing those views.
- **Transform boundaries**: The playfield uses noteskin transforms and `gfx_util.transform(self.transform)` in several places. Retained views need a single transform adapter so hit-testing and drawing agree.
- **Draw ordering**: Notes, snap grid, waveform, onsets, selection rectangle, notifications, and overlay chrome currently rely on a precise manual order. Phase 5 should introduce retained layers rather than one monolithic playfield view.
- **Service contracts**: Existing action, scroll-input, selection, note-drag, playfield, playfield-hit-test, playfield-input-state, and playfield-input services are good boundaries. Phase 5 should move the legacy adapter queries into retained event handlers and pass explicit events into services.
- **Verification**: Add tests for retained playfield event routing before swapping rendering. At minimum cover note add/drag/drop, selection rectangle, snap-grid scroll, tool hotkeys, command hotkeys, and resource-not-loaded overlays.
...
During editor enter, `EditorScreenLoadService` attaches retained editor chrome views from `ui.views.EditorView.retained`: `EditorWaveformView`, `EditorOnsetsView`, `EditorOnsetsDistView`, `EditorFooterView`, and `EditorOverlayView`. `EditorScreenFrameService` draws those through `yi.Screen` when attached and keeps the old direct draw functions only as a fallback for focused tests or partial setup. The retained wrappers are migration anchors: replace their delegated legacy draw functions with native retained child views incrementally, without changing editor model/service behavior.
...
Foreground hotkeys are dispatched through `EditorActionService`; views pass key state and a narrow `EditorActionContext` into the service instead of giving it `EditorModel`, `EditorController`, or notification objects directly. Footer playback controls and rate mutation live in `EditorFooterService` through `EditorFooterContext`. Snap-grid scroll and drag behavior is owned by `EditorScrollInputService` through `EditorScrollInputContext`, including fine-scroll speed override, pause/resume while dragging, snap changes, and speed changes. Chart-slider timeline data and drag behavior is owned by `EditorChartSliderService` through `EditorChartSliderContext`. Waveform sample-window caching and line generation live in `EditorWaveformService` through `EditorWaveformContext`; the view only draws the returned line data. NCBT onset debug view data lives in `EditorOnsetsService` through `EditorOnsetsContext`. Timing overlay navigation, active interval vertex commands, the show-timings setting, and comment visual-point lookup live in `EditorTimingOverlayService` through `EditorTimingOverlayContext`. Audio overlay status display, such as playing source count and offsync, lives in `EditorAudioOverlayService` through `EditorAudioOverlayContext`; audio/editor/waveform setting writes live in `EditorAudioSettingsOverlayService` through `EditorAudioSettingsOverlayContext`. Notes overlay speed, snap, lock-snap, tool selection, snap limit, selected-note summary, and note tool hotkey data live in `EditorNotesOverlayService` through `EditorNotesOverlayContext`. Info overlay metadata editing and save/export command wiring live in `EditorInfoOverlayService` through `EditorInfoOverlayContext`. BMS overlay state and export command wiring live in `EditorBmsOverlayService` through `EditorBmsOverlayContext`; exporter internals remain deferred. Overlay-only actions such as preview time, comments, selected-note commands, NCBT detect/apply commands, total-beat display data, and BMS offset/tempo controls live in `EditorOverlayActionService` through `EditorOverlayActionContext` rather than receiving the whole `EditorModel`.

Legacy playfield note interactions are routed through `EditorPlayfieldService` and `EditorPlayfieldContext`: notes-tab activation, note-tool/select-tool checks, selection rectangle lookup, add-note commands, note select/grab/remove commands, grabbed-note drop, and selection end. Mouse decision routing is owned by `EditorPlayfieldInputService`: the legacy rhythm view supplies hit-test and button facts, and the service turns them into playfield commands. Legacy hit-test fact construction is isolated in `EditorPlayfieldHitTestService`, which wraps immediate-mode hover checks and noteskin column bounds. Legacy mouse button state is isolated in `EditorPlayfieldInputStateService`. The rhythm view should not depend on `just` directly; retain `just` usage only in these adapter services until retained playfield input exists.
