...
- **Decision**: `EditorSequenceView` owns editor-only note-skin sequence iteration. `EditorRhythmView` is drawing-only and only overrides the editor note source. `EditorPlayfieldView` owns retained mouse events, selection-rectangle drawing, add-note/select-note/remove-note/drop-note routing, and calls `EditorPlayfieldInputService` with explicit input facts after notes have drawn their hover state.
- **Consequence**: Editor playfield input no longer depends on `just` or immediate-mode mouse state. The remaining playfield migration is replacing shared note-skin view objects or moving their hover facts behind explicit renderer contracts.
...
- `EditorOverlayView`: menus, tool selection, timing/audio/note/BMS panels, and resource-loading overlay.
...
Views should read lifecycle/resource state through `EditorModel` accessors instead of raw fields. Editor retained views use `gui.View` input events and `yi.views.editor.EditorGui` controls for buttons, sliders, checkboxes, text inputs, combo cycling, and tabs. Do not add new editor UI behavior through `imgui` or `just`; model readiness, waveform, visual access, modifier state, and small UI mutations should stay behind model or service methods so runtime state does not drift into ad hoc fields.
...
Playfield note interactions are routed through `EditorPlayfieldService` and `EditorPlayfieldContext`: notes-tab activation, note-tool/select-tool checks, selection rectangle lookup, add-note commands, note select/grab/remove commands, grabbed-note drop, and selection end. Mouse decision routing is owned by `EditorPlayfieldInputService`: `EditorPlayfieldView` supplies retained mouse-event facts, and the service turns them into playfield commands. Hit-test fact construction is isolated in `EditorPlayfieldHitTestService`, which reads note hover facts from note rendering and column bounds from the active note skin.
