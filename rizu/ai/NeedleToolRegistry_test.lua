...
function test.context_allowlist(t)
	...
	t:assert(tools.routing_tools_json:find('"name":"set_playback_rate"', 1, true))
	t:eq(tools.routing_tools_json:find('"parameters"', 1, true), nil)
	t:eq(tools.routing_tools_json:find('"type":"number"', 1, true), nil)
	t:eq(tools.routing_tools_json:find('"required":true', 1, true), nil)
