...
---@class rizu.ai.RuntimeInspectionResult
---@field target string
---@field inspection rizu.ai.RuntimeInspection

---@param text string
---@return rizu.ai.RuntimeInspectionResult
local function decodeResult(text)
...
function test.inspects_nested_runtime_values(t)
	...
	local result = decodeResult(tool:execute({target = "game.service", depth = 2, max_items = 20}))
	---@type {[string]: rizu.ai.RuntimeInspection}
	local items = {}
...
function test.bounds_depth_and_items(t)
	...
	local result = decodeResult(tool:execute({target = "game.values", depth = 1, max_items = 1}))
	t:eq(#result.inspection.items, 1)
	t:eq(result.inspection.truncated, true)

	result = decodeResult(tool:execute({target = "game.values", depth = 0, max_items = 10}))
	t:eq(result.inspection.truncated, true)
