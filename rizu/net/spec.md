...
- `restart_game` queues `love.event.quit("restart")`; the current response is written before the main loop consumes the quit event, after which clients must reconnect to the new process.
