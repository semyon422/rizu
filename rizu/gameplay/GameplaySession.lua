...
function GameplaySession:setPlayType(play_type)
	...
	if play_type == "auto" and self.rhythm_engine.catch_rules then
		self.replay_player = ReplayPlayer(CatchRules.autoplay(self.rhythm_engine.catch_rules.chart))
	elseif play_type == "auto" and self.rhythm_engine.aim_rules then
...
function GameplaySession:update(current_time)
	...
	if self.play_type == "auto" and (re.aim_rules or re.catch_rules) then
...
function GameplaySession:receive(event, current_time)
	...
	re:setGlobalTime(current_time)
	re:receive(event)
	if re.aim_rules or re.catch_rules then
