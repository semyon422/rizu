...
Implement the first Aim vertical slice: native osu! circles, manual input, autoplay, and local diagnostic replay, reusing the existing gameplay clock/audio/session without mania scoring or skin requirements. See [../modes/spec.md](../modes/spec.md) for the larger four-mode plan.
...
- Only circle-only charts are playable. Sliders, spinners, empty charts, rates outside 0.25x–4x, column reordering, and chart modifiers are rejected with a loading-screen diagnostic. Press Escape to return.
- The temporary playfield shows numbered circles, approach rings, a cursor, HIT/MISS feedback, counters, and pause state. Use Z/X or the left/right mouse buttons. These prototype bindings are fixed, not edited by the mania binding panel.
...
- The initial object DTO preserves circle position/time/type/sounds and identifies unsupported object kinds; full slider geometry is deferred. Missing AR uses OD. Settings outside 0–10 are rejected for this prototype.
...
- Add sliders and spinners without discarding source geometry; implement stacking and review overlap/note-lock compatibility.
