...
---@field format "rizu-taiko-1"|"rizu-catch-1"|"rizu-aim-circles-1"|"rizu-aim-sliders-1"|"rizu-aim-spinners-1"|"rizu-aim-stacking-1"|"rizu-aim-tracking-1"
---@field hash string
---@field index integer
---@field rate number
...
---@param mode "aim"|"catch"|"taiko"?
---@param fs fs.IFilesystem
function ReplayStore:new(fs, mode)
	self.fs = fs
	self.mode = mode or "aim"
	assert(self.mode == "aim" or self.mode == "catch" or self.mode == "taiko", "Invalid replay mode.")
...
function ReplayStore:save(session)
	...
	assert((self.mode == "taiko" and re.taiko_rules or self.mode == "catch" and re.catch_rules or self.mode == "aim" and re.aim_rules) and session.play_type == "manual", "only manual experimental attempts can be saved")
	local meta = re.chartmeta
	local path = self:path(meta.hash, meta.index)
	---@type rizu.aim.ReplayData
	local data = {
		format = self.mode == "taiko" and "rizu-taiko-1" or self.mode == "catch" and "rizu-catch-1" or "rizu-aim-tracking-1", hash = meta.hash, index = meta.index,
...
function ReplayStore:load(hash, index)
	...
	assert((replay.format == "rizu-taiko-1" or replay.format == "rizu-catch-1" or replay.format == "rizu-aim-circles-1" or replay.format == "rizu-aim-sliders-1" or replay.format == "rizu-aim-spinners-1" or replay.format == "rizu-aim-stacking-1" or replay.format == "rizu-aim-tracking-1") and replay.hash == hash and replay.index == index, "Incompatible experimental replay.")
	...
	for _, frame in ipairs(frames) do
		...
		assert(event.id >= 0 and event.id <= (self.mode == "catch" and 6 or 4) and (event.column == 1 or event.column == 2), "Invalid experimental input channel.")
