...
---@field format "rizu-aim-circles-1"|"rizu-aim-sliders-1"|"rizu-aim-spinners-1"
---@field hash string
---@field index integer
---@field rate number
...
function ReplayStore:save(session)
	...
	local data = {
		format = "rizu-aim-spinners-1", hash = meta.hash, index = meta.index,
...
function ReplayStore:load(hash, index)
	...
	assert((replay.format == "rizu-aim-circles-1" or replay.format == "rizu-aim-sliders-1" or replay.format == "rizu-aim-spinners-1") and replay.hash == hash and replay.index == index, "Incompatible Aim replay.")
