...
Requirements for the Aim, Catch, Taiko, and SDVX prototypes are tracked in [modes/spec.md](modes/spec.md). The circle/slider Aim implementation and its limitations are documented in [aim/spec.md](aim/spec.md); the remaining modes and Aim spinners are still planned. The work covers core mechanics, ordinary chart selection, autoplay/replays, and investigation for future skinning work without mode-specific scoring or score submission.
