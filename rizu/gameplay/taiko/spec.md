...
- Mode identity changes for newly decoded native Taiko from legacy `2key` to `1taiko`. Taiko objects use `Note.data`, shared parameters use `Chart.data`, and distinct visual points preserve coincident notes through RefChart workers. Old library rows are not silently migrated: they remain searchable as `2key`, but ordinary loading re-decodes the source and uses Taiko rules. Reimport/reindex is needed for `1taiko` search and the experimental label. Legacy 2K competitive replays are not a Taiko replay format and are not migrated.
