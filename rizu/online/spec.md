...
- `OnlineClient` stores client-observed online state such as the current user, server connection status, and leaderboard data. Consumers subscribe with `onChanged`/`offChanged`; it emits `user_changed` when the runtime user changes, `connection_changed` when the websocket connects or disconnects, `authentication_resolved` after initial token restoration has completed, and `login_started`/`login_succeeded`/`login_failed` for interactive login state. Reassigning the same connection or user state does not emit a duplicate event.
- `MultiplayerModel` tracks client-side multiplayer state and coordinates room chart selection/download actions. Its `MultiplayerClient` exposes `onChanged`/`offChanged`; server pushes emit `rooms_changed`, `users_changed`, and `room_users_changed` so UI consumers do not poll multiplayer arrays, and a rejected join emits `join_failed`.
