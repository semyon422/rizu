...
- `OnlineClient` stores client-observed online state such as the current user, server connection status, and leaderboard data. Consumers subscribe with `onChanged`/`offChanged`; it emits `user_changed` when the runtime user changes and `connection_changed` when the websocket connects or disconnects. Reassigning the same state does not emit a duplicate event.
