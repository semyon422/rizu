...
local BYTE_x, BYTE_X, BYTE_b, BYTE_B = sbyte("x"), sbyte("X"), sbyte("b"), sbyte("B")
local BYTE_i, BYTE_I = sbyte("i"), sbyte("I")
...
local function to_bin(b)
   if b == BYTE_0 or b == BYTE_0 + 1 then
      return b-BYTE_0
   end
end
...
local function lex_number(state, b)
   ...
   if b == BYTE_0 then
      ...
      if b == BYTE_x or b == BYTE_X then
         ...
         is_digit = to_hex
         b = next_byte(state)
      elseif b == BYTE_b or b == BYTE_B then
         is_digit = to_bin
         is_binary = true
         b = next_byte(state)
      else
   ...
   if not is_binary and b == BYTE_DOT then
      -- Fractional part.
      is_float = true
   ...
   if not is_binary and (b == exp_lower or b == exp_upper) then
      -- Exponent part.
      is_float = true
      b = next_byte(state)
