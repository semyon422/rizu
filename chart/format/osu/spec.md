...
- `AimDecoder` carries source-order slider control points (including the head), curve type, declared pixel length, span count, timing points, SliderMultiplier, SliderTickRate, and format version, plus StackLeniency (default 0.7 when absent). `RefChart` carries this plain data across workers; runtime path objects are constructed separately.
...
## Direct Chart Decoding And Rules

`AimDecoder`, `CatchDecoder`, and `TaikoDecoder` populate ordinary notes and `Chart.data`; there are no mode-specific Chart classes. Catch uses a temporary common Chart from the shared Aim source decoder to derive fruit streams, not an Aim gameplay DTO. Derived objects receive separate visual points in chronological, stable order.

The three rules and autoplay entry points accept `chart.Chart`. Runtime `objects` lists reference selected note payloads; parameters are read from `chart.data`. They do not call `ModeNotes.read`. Stacking copies the common Chart and mutates only that attempt's geometry; shared source notes and retry state remain unchanged.
