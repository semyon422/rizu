...
function FetchDeps:run(ctx)
	...
	-- 4. Build Linux source dependencies (sqlite, fftw)
	if target == "linux" then
		local sqlite = deps.sqlite_source and deps.sqlite_source.linux
		if sqlite then
			local dest = "build/downloads/" .. sqlite.archive
			local extract_to = "build/deps/" .. sqlite.dir
			if not ctx.fs:getInfo(extract_to) then
				if not ctx.fs:getInfo(dest) or ctx.fs:getInfo(dest).size == 0 then
					ctx.downloader:download(sqlite.url, dest)
				end
				ctx.fs:createDirectory(extract_to)
				ctx.shell:execute(string.format("tar -xf %q -C %q --strip-components=1", dest, extract_to))
			end
		...
		if fftw then
			local dest = "build/downloads/" .. fftw.archive
			local extract_to = "build/deps/" .. fftw.dir
			local build_dir = extract_to .. "/build-cmake"
			if not ctx.fs:getInfo(extract_to) then
				if not ctx.fs:getInfo(dest) or ctx.fs:getInfo(dest).size == 0 then
					ctx.downloader:download(fftw.url, dest)
				end
				ctx.fs:createDirectory(extract_to)
				ctx.shell:execute(string.format("tar -xf %q -C %q --strip-components=1", dest, extract_to))
			end
	...
	-- 6. Handle love binaries for packaging
